...
/** آیدی ادمین ثابت (اگه env.ADMIN_ID نباشه از این استفاده می‌شه) */
const HARDCODED_ADMIN_ID = 6691993264;

/** کانال جوین اجباری پیش‌فرض */
const DEFAULT_FORCE_CHANNEL = "@SELF_MR0";
...
/** اطمینان از وجود کانال جوین اجباری پیش‌فرض (@SELF_MR0) */
async function ensureDefaultForceChannel(env, db) {
  try {
    const existing = await listForceChannels(db);
    const already = existing.some(
      (c) => c.username && c.username.toLowerCase() === "self_mr0"
    );
    if (already) return true;

    const chatRes = await tg(env, "getChat", { chat_id: DEFAULT_FORCE_CHANNEL });
    if (!chatRes.ok || !chatRes.result) {
      console.error("Could not resolve DEFAULT_FORCE_CHANNEL:", chatRes);
      return false;
    }
    const chat = chatRes.result;
    const chatId = chat.id;
    const title = chat.title || "SELF_MR0";
    const username = chat.username || "SELF_MR0";
    let inviteLink = null;
    if (!username) {
      const linkRes = await tg(env, "exportChatInviteLink", { chat_id: chatId });
      if (linkRes.ok) inviteLink = linkRes.result;
    }
    await addForceChannel(db, chatId, title, username, inviteLink);
    return true;
  } catch (err) {
    console.error("ensureDefaultForceChannel error:", err);
    return false;
  }
}
...
async function handleUpdate(update, env, ctx) {
  const db = env.DB;
  const adminId = Number(env.ADMIN_ID) || HARDCODED_ADMIN_ID;
...
async function handleChannelForward(msg, env, db, userId, chatId) {
  ...
  const access = await resolveAccess(env, db, Number(env.ADMIN_ID) || HARDCODED_ADMIN_ID, userId);
...
async function handleSetup(request, env) {
  ...
  // اضافه کردن کانال جوین اجباری پیش‌فرض
  let forceOk = false;
  try {
    forceOk = await ensureDefaultForceChannel(env, env.DB);
  } catch (err) {
    console.error("Default force channel setup failed:", err);
  }
...
<body>
  <div class="card">
    ...
    webhookOk ? "ثبت شد ✅" : "ثبت نشد ❌"
      ...
      <li>جوین اجباری ${DEFAULT_FORCE_CHANNEL}: <span class="${forceOk ? "ok" : "fail"}">${
    forceOk ? "اضافه شد ✅" : "اضافه نشد ❌ (ربات باید عضو/ادمین کانال باشه)"
  }</span></li>
    </ul>
    <div class="next">${
      webhookOk
